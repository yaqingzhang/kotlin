...
private fun transformKaptToolArgs(args: List<String>, messageCollector: MessageCollector, isTest: Boolean): List<String> {
    ...
    while (iterator.hasNext()) {
        ...
        if (option.pluginOption == KaptCliOption.TOOLS_JAR_OPTION) {
        ...
        if (option.pluginOption == KaptCliOption.APT_MODE_OPTION) {
            aptModePassed = true
        }

        if (option.pluginOption == KaptCliOption.VERBOSE_MODE_OPTION) {
            kaptVerboseModePassed = true
        }
