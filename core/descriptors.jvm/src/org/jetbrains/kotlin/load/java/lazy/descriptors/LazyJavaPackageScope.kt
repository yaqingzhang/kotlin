        ...
        c.storageManager.createMemoizedFunctionWithNullableValues<FindClassRequest, ClassDescriptor> classByRequest@{ request ->
            ...
            // These branches should be semantically equal, but the first one could be faster
                if (request.javaClass != null)
                    c.components.kotlinClassFinder.findKotlinClassOrClassFileContent(request.javaClass)
                else
                    c.components.kotlinClassFinder.findKotlinClassOrClassFileContent(requestClassId)
                    ...
                    val javaClass =
                        request.javaClass ?: c.components.finder.findClass(
                            JavaClassFinder.Request(
                                requestClassId,
                                kotlinClassOrClassFileContent?.safeAs<KotlinClassFinder.KotlinClassOrClassFileContent.ClassFileContent>()
                                    ?.content
                            )
                        )
