...
class LateinitLowering(
    ...
    override fun lower(irFile: IrFile) {
        irFile.transformChildrenVoid(object : IrElementTransformerVoid() {
            ...
            override fun visitCall(expression: IrCall): IrExpression {
                ...
                val receiver = (expression.extensionReceiver ?: return expression) as? IrPropertyReference ?: return expression

                if (!checkIsInitializedProperty(expression)) return expression

                val backingField = receiver.getter?.owner?.resolveFakeOverride()?.correspondingProperty?.backingField!!

                return expression.run { context.createIrBuilder(symbol, startOffset, endOffset) }.run {
                    irNotEquals(irGetField(receiver.dispatchReceiver, backingField), irNull())
                }
            }
    ...
    private fun checkIsInitializedProperty(expression: IrCall): Boolean {
        val receiver = (expression.extensionReceiver as? IrPropertyReference) ?: return false

        val target = expression.symbol.owner as? IrSimpleFunction ?: return false

        val property = target.correspondingProperty ?: return false

        if (property.name.asString() != "isInitialized") return false

        val parent = (property.parent as? IrPackageFragment) ?: return false

        if (parent.fqName.asString() != "kotlin") return false

        return receiver.getter?.owner?.resolveFakeOverride()?.correspondingProperty?.isLateinit == true
    }
